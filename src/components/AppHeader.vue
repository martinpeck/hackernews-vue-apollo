  ...
  <div class="flex pa1 justify-between nowrap orange">
    <div class="flex flex-fixed black">
      ...
      <div class="flex" v-if="userId">
        <div class="ml1">|</div>
        <router-link :to="{ name: 'create' }" class="ml1 no-underline black">submit</router-link>
      </div>
    </div>
    <div class="flex flex-fixed">
      <div v-if="userId" class="ml1 pointer black" @click="logout()">logout</div>
      <router-link v-else :to="{ name: 'login' }" class="ml1 no-underline black">login</router-link>
    </div>
  </div>
</template>
